...
void OBSBasic::InitAgoraServiceSettings()
{
	...
	if (strcmp(speakers, "Mono") == 0) { //
		...
		if (m_agoraSettings.bAudioHighQuality) {
			audio_profile =
				5; //AUDIO_PROFILE_MUSIC_HIGH_QUALITY_STEREO
		} else {
			audio_profile = 3; //AUDIO_PROFILE_MUSIC_STANDARD_STEREO
		}
	}

	obs_data_set_int(settings, "agora_audio_profile", audio_profile);
...
void OBSBasic::AgoraTokenPrivilegeWillExpire(void *data, calldata_t *params)
{
	QMetaObject::invokeMethod(App()->GetMainWindow(),
