...
public:
    ...
    */
	static std::string buildTokenWithUid(const char *appId,
   ...
   */
	static std::string buildTokenWithUserAccount(
      ...
      const std::string& userAccount,*/
		const char *appId, const char *appCertificate,
		const char *channelName, const char *userAccount, UserRole role,
		uint32_t privilegeExpiredTs = 0);
};
                                  ...
                                  uint32_t privilegeExpiredTs*/)
{
	...
	return RtcTokenBuilder::buildTokenWithUserAccount(appId, appCertificate,
							  channelName,
							  str.c_str(), role,
							  privilegeExpiredTs);
}
...
inline std::string RtcTokenBuilder::buildTokenWithUserAccount(
	/* const std::string& appId,
                                          ...
                                          const std::string& channelName,
                                          const std::string& userAccount,*/
	const char *appId, const char *appCertificate, const char *channelName,
	const char *userAccount, UserRole role, uint32_t privilegeExpiredTs)
{
	AccessToken generator(appId, appCertificate, channelName,
			      privilegeExpiredTs,  userAccount);
	generator.AddPrivilege(AccessToken::Privileges::kJoinChannel,
			       privilegeExpiredTs);
	if (role == Role_Attendee || role == Role_Publisher ||
	    role == Role_Admin) {
		generator.AddPrivilege(
			AccessToken::Privileges::kPublishAudioStream,
			privilegeExpiredTs);
		generator.AddPrivilege(
			AccessToken::Privileges::kPublishVideoStream,
			privilegeExpiredTs);
		generator.AddPrivilege(
			AccessToken::Privileges::kPublishDataStream,
			privilegeExpiredTs);
	}
	//generator.SetTokenExpiredTs(privilegeExpiredTs);
